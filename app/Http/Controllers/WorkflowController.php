...
class WorkflowController extends Controller
{
    ...
    public function postComment($id, Request $request)
    {
        ...
        if ($requisition->Status === $username) {
            ...
            } elseif ($username === $requisition->approvers->Approver2 || ($requisition->Reassigned && $requisition->ReassignedPosition === 'Approver2')) {
                $requisition->ApprovedComments2 = $comment;
            } elseif ($username === $requisition->approvers->Approver3 || ($requisition->Reassigned && $requisition->ReassignedPosition === 'Approver3')) {
                $requisition->ApprovedComments3 = $comment;
            } elseif ($username === $requisition->approvers->Approver4 || ($requisition->Reassigned && $requisition->ReassignedPosition === 'Approver4')) {
                $requisition->ApprovedComments4 = $comment;
            } elseif ($username === $requisition->approvers->Approver5 || ($requisition->Reassigned && $requisition->ReassignedPosition === 'Approver5')) {
                $requisition->ApprovedComments5 = $comment;
            } elseif ($username === $requisition->approvers->ApproverTE || ($requisition->Reassigned && $requisition->ReassignedPosition === 'ApproverTE')) {
                $requisition->ApprovedCommentsTE = $comment;
            } elseif ($username === $requisition->approvers->ApproverFinal || ($requisition->Reassigned && $requisition->ReassignedPosition === 'ApproverFinal')) {
    ...
    private function reassignRequisition($id, $from_user, $to_user, $shouldBypassEmail, $requisition = null)
    {
        ...
        } elseif ($this->checkApprover2($requisition, $from_user)) {
            $requisition->ReassignedPosition = 'Approver2';
        } elseif ($this->checkApprover3($requisition, $from_user)) {
            $requisition->ReassignedPosition = 'Approver3';
        } elseif ($this->checkApprover4($requisition, $from_user)) {
            $requisition->ReassignedPosition = 'Approver4';
        } elseif ($this->checkApprover5($requisition, $from_user)) {
            $requisition->ReassignedPosition = 'Approver5';
        } elseif ($this->checkTEApprover($requisition, $from_user)) {
        ...
        if (! $shouldBypassEmail && $this->approverWantsEmail($nextApproverEmail)) {
            Mail::to($nextApproverEmail)->send(new BTNextApprover($requisition));
        }
    }
    ...
    private function setApproverFields($requisition, $status, $username)
    {
        ...
        } elseif ($requisition->ApprovedBy2 === '' && $requisition->approvers->Approver2 === $username) {
            return $this->setApprover2($requisition, $status, $username);
        } elseif ($requisition->ApprovedBy3 === '' && $requisition->approvers->Approver3 === $username) {
            return $this->setApprover3($requisition, $status, $username);
        } elseif ($requisition->ApprovedBy4 === '' && $requisition->approvers->Approver4 === $username) {
            return $this->setApprover4($requisition, $status, $username);
        } elseif ($requisition->ApprovedBy5 === '' && $requisition->approvers->Approver5 === $username) {
            return $this->setApprover5($requisition, $status, $username);
        } elseif ($requisition->ApprovedByTE === '' && $requisition->approvers->ApproverTE === $username && $requisition->Technology === 'TE') {
            return $this->setApproverTE($requisition, $status, $username);
        } elseif ($requisition->FinalApprovedBy === '' && $requisition->approvers->ApproverFinal === $username && $requisition->Status === $username) {
            return $this->setApproverFinal($requisition, $status, $username);
        }
    ...
    private function isRejected($requisition)
    {
        ...
        if ($requisition->ApprovedStatus2 === 'Rejected') {
            return true;
        }
        if ($requisition->ApprovedStatus3 === 'Rejected') {
            return true;
        }
        if ($requisition->ApprovedStatus4 === 'Rejected') {
            return true;
        }
        if ($requisition->ApprovedStatus5 === 'Rejected') {
            return true;
        }
        if ($requisition->ApprovedStatusTE === 'Rejected') {
            return true;
        }
        if ($requisition->ApprovedFinalStatus === 'Rejected') {
            return true;
        }
