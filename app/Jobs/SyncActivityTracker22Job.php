...
class SyncActivityTracker22Job implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle(ATWorkflowService22 $at)
    {
        $this->at = $at;
    ...
    private function runImport()
    {
        foreach ($this->requisitions as $recId => $requisition) {
            ...
            $exisitngRec = ATRequisition22::where('RecID', '=', $requisition['RecID'])->first();
            if ($exisitngRec !== null) {
                ...
                $requisition = $this->requisitionBuilder($requisition, new ATRequisition22(), $this->createExclusions);
                ...
                if(isset($requisition->Status) && $requisition->Status !== 'Completed' && $requisition->Status !== 'Approved' && $requisition->Status !== 'Rejected'){
                    Mail::to($this->getNextApproverEmail($requisition))->send(new ATNextApprover22($requisition));
                }
            }
    ...
    {
        ATRequisitionItem22::where('RequisitionNo', $requisition['RequisitionNo'])->delete();
        return $this;
    }
    ...
    private function constructRequisitionItems($requisition)
    {
        ...
        foreach($items as $recordId => $recItem) {
            ...
            $requisitionItem = ATRequisitionItem22::where(['RequisitionNo'=> $recId, 'fmId' => $recItem['id']])->first();
            if($requisitionItem !== null) {
                $this->requisitionItemBuilder($requisitionItem, $recItem);
            } else {
                $this->requisitionItemBuilder(new ATRequisitionItem22(), $recItem);
            }
    ...
    {
        SendActivityTrackerApprovals22Job::dispatchNow();
        return $this;
    }
