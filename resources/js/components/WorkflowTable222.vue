<template>
    <div class="w-full m-3">
        ...
        <t-modal ref="requisition-modal" :width="width">
            <workflow-requisition-modal-22 :imgurl="imgurl" :row="activeRow" :actor="actor" v-on:load="load"/>
        </t-modal>
        <t-modal ref="reassign-modal" :width="450">
            <workflow-reassign-modal-22 :row="activeRow" :rowIndex="activeRow.id" :actor="actor" v-on:load="load"/>
        </t-modal>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            loadActive(){
                ...
                this.loading = true;
                this.actor = "";
                axios.get('/staff/22/workflowBackend/budgetTracker')
                    .then(response => {
            ...
            loadCompleted(){
                ...
                this.noRequisitions = false;
                //this.actor = "";
                axios.get('/staff/22/btWorkflowBackend/Approved' + ((this.actor.length > 0) ? "/" + this.actor : ""))
                    .then(response => {
                        if(response.data.length < 1){
                            this.noRequisitions = true;
                        } else {
            ...
            loadRejected(){
                ...
                this.noRequisitions = false;
                //this.actor = "";
                axios.get('/staff/22/btWorkflowBackend/Rejected' + ((this.actor.length > 0) ? "/" + this.actor : ""))
                    .then(response => {
                        if(response.data.length < 1){
                            this.noRequisitions = true;
                        } else {
            ...
            impersonate(){
                ...
                this.loading = true;
                this.noRequisitions = false;
                axios.get('/staff/22/workflowBackend/budgetTracker/user/' + this.actor)
                    .then(response => {
                        if(response.data.length < 1){
                            this.noRequisitions = true;
                        } else {
            ...
            manualSync(){
                if(!this.loadingSync){
                    ...
                    axios.get('/staff/22/workflowBackendSync')
                        .then(res=>{
                            console.log(res);
                            if(this.actor === ""){
                                this.loadActive();
