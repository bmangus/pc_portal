    ...
    <div>
        ...
        <div ref='printarea' class="font-sans">
            ...
            <t-modal ref="modal-forward" :width="width">
                <at-workflow-forward-modal :row="row" :rowIndex="rowIndex" :actor="actor" v-on:load="hideModal"/>
            </t-modal>
            <t-modal ref="rejection-modal" :width="width">
                <at-workflow-rejection-modal :requisition-id="row.pk" :actor="actor" v-on:load="emitLoad"/>
            </t-modal>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        name: 'at-workflow-requisition-modal',
        ...
        methods: {
            approveReq(id){
                ...
                axios.get('/staff/atWorkflowBackend/' + id + '/Approved' + actorString)
                    .then(res=>{
            ...
            rejectReq(id, mid){
                let actorString = (this.actor !== "") ? '/' + this.actor : "";
                axios.get('/staff/atWorkflowBackend/' + id + '/Rejected' + actorString)
                    .then(res=>{
                        this.$refs['modal'+mid].hide();
                        this.$emit('load', true);
                    })
            ...
            print(id){
                window.open('/staff/atWorkflowPDF/download/' + id, '_blank');
            },
            openModal(){
