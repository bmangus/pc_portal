<template>
    <div>
        ...
        <div ref='printarea' class="font-sans">
            ...
            <workflow-comment-22 :requisitionId="row.pk" :actor="actor" :status="row.Status"/>
            <t-modal ref="modal-forward" :width="width">
                <workflow-forward-modal-22 :row="row" :rowIndex="row.id" :actor="actor" v-on:load="hideModal"/>
            </t-modal>
            <t-modal ref="rejection-modal" :width="width">
                <workflow-rejection-modal-22 :requisition-id="row.pk" :actor="actor" v-on:load="emitLoad"/>
            </t-modal>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        name: 'workflow-requisition-modal-22',
        ...
        methods: {
            approveReq(id){
                if(!this.requisitionApproving){
                    ...
                    axios.get('/staff/22/workflowBackend/budgetTracker/' + id + '/Approved' + actorString)
                        .then(res=>{
            ...
            print(id){
                window.open('/staff/22/workflowPDF/download/' + id, '_blank');
            },
            executePrint(){
                this.printHtml2Canvas().then((output)=>{
                    const formData = new FormData();
                    formData.append('dataURL', output);
                    axios.post('/staff/22/workflowPDF/send/budgetTracker', formData)
